...
class FileManagerSkill(BaseSkill):
    ...
    async def _list_directory(self, path: str, pattern: str = "*") -> SkillResult:
        """列出目录内容"""
        if not self._is_path_safe(path):
            return SkillResult(success=False, output=None, error="访问被拒绝：路径不在安全范围内")
        ...
        try:
            ...
            return SkillResult(success=True, output=items[:100])  # 限制数量
        except Exception as e:
            return SkillResult(success=False, output=None, error=str(e))
    ...
    async def _search_files(self, directory: str, pattern: str, recursive: bool = True) -> SkillResult:
        """搜索文件"""
        if not self._is_path_safe(directory):
            return SkillResult(success=False, output=None, error="访问被拒绝：路径不在安全范围内")
        
        dir_obj = Path(directory).expanduser()
        
        if not dir_obj.exists() or not dir_obj.is_dir():
            return SkillResult(success=False, output=None, error=f"目录不存在: {directory}")
